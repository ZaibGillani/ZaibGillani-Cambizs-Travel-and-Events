...
class EventController extends Controller
{
	public function basic_email() {
      ...
      Event::send(['text'=>'mail'], $data, function($message) {
   ...
   public function attachment_email() {
	...
	function event_list(){
		$event_data = DB::table('events_table')->where('status', '1')->orderBy('id',"DESC")->take(5)->get();
	...
	function event_postpond(Request $request){
		$gid = $request->input('id'); 
		$status = 0;
		$ev_data = DB::table('events_table')
						->where('id', $gid)
						->get();
		if(!empty($ev_data)){
			$status = $ev_data[0]->status;
			if($status==''){
				$status = 1;
			} else if($status=='1'){
				$status = '0';
        ...
        ->where('id', $gid)  // find your user by their email
	...
	function event_user_setting($id){
		...
		if($user_id==$id){
			...
			return view('404', ["user_id"=>$id]);
		}	
	}
	
	function get_event_manager(Request $request){
		$id = Auth::user()->id;
		$get_role = '';
		if($id!=''){
		...
		if($get_role=="role_event"){
			$event_data = DB::table('events_table')->orderBy('id',"DESC")->where('user_id', $id)->cursorPaginate(10);
			return response()->view('event_user/my_event', ['event_data' => $event_data,"user_id"=>$id]);
		} else{
			return view('404', ["user_id"=>$id]);
		}
	...
	function featured_events(Request $request){
		$event_data = DB::table('events_table')->where('status', '1')->orderBy('id',"DESC")->take(5)->get();
		return view('featured_event', ['event_data' => $event_data]);
	}
